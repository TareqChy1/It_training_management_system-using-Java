...
public class PurchaseFrame extends JFrame implements ActionListener
{
	...
	private User user;
	private PurchaseRepo er=new PurchaseRepo();
	...
	public PurchaseFrame(User user)
	{
		...
		cIdTF.setBounds(220,150,100,30);
		panel.add(cIdTF);
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		else if(command.equals(insertBtn.getText()))
		{
			...
			if(course !=null && e != null  && course.getCourseId().equals(e.getCourseId())){
			try{
			...
			p.setStudentName(e.getStudentName());
			p.setAmount(course.getCourseFee());
			
	
			er.insertInDB(p);
			
			JOptionPane.showMessageDialog(this, "Inserted.ID:"+x);
			}
			catch(Exception ro)
			{
		...
		else if(command.equals(updateBtn.getText()))
		{
			...
			CourseRepo yr_1 = new CourseRepo();
			Course course_1 = yr_1.searchCourse(cIdTF.getText());
			StudentRepo ur_1 = new StudentRepo();
			Student e_1 = ur_1.searchStudent(stdIdTF.getText());
			if(course_1 !=null && e_1 != null  && course_1.getCourseId().equals(e_1.getCourseId()))
			{
